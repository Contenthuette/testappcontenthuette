...
export default function AdminEventMgmt() {
  const events = useQuery(api.admin.listEvents, {});

  return (
    <SafeAreaView style={styles.safe} edges={["top"]}>
      <View style={styles.header}>
        <TouchableOpacity onPress={() => safeBack("event-mgmt")} style={styles.backBtn}>
          <SymbolView name="chevron.left" size={20} tintColor={colors.black} />
        </TouchableOpacity>
        <Text style={styles.title}>Eventverwaltung</Text>
      </View>
      <FlatList
        data={events}
        ...
        renderItem={({ item }) => (
          <View style={styles.row}>
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.tickets}>{item.soldTickets}/{item.totalTickets} Tickets</Text>
            </View>
          </View>
        )}
        ListEmptyComponent={events === undefined ? <ActivityIndicator style={{ marginTop: 40 }} color={colors.gray400} /> : null}
      />
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  tickets: { fontSize: 12, color: colors.gray400, marginTop: 2 },
});
